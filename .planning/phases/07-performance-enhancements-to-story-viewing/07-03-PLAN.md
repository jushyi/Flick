...
must_haves:
  ...
  key_links:
    - from: "src/screens/PhotoDetailScreen.js"
      ...
      via: "subscribePhoto pause/resume during transitions"
      pattern: "pauseSubscription|resumeSubscription"
...
**C. Next-friend photo prefetching (PhotoDetailContext.js + usePhotoDetailModal.js):**

11. In `PhotoDetailContext.js`, find where the story sequence is stored. Look for `storySequenceRef` or equivalent. Add a function or value that exposes the next friend's first photo URL:
...
12. In `usePhotoDetailModal.js`, add a prefetch effect that runs when currentIndex changes (at the last photo of current friend, prefetch next friend's first photo):
    ```javascript
