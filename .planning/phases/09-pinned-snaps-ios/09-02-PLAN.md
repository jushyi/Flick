...
**Wave 0 test coverage note:** RESEARCH.md identifies three Wave 0 test gaps (`usePinPreference.test.js`, `liveActivityService.test.js`, extend `snapService.test.js`). Tests are co-created alongside implementation in Plans 02, 03, and 04 rather than in a separate Wave 0 plan. This is acceptable because: (1) the services and hooks being tested do not exist yet — no interfaces or module structure to write RED tests against, (2) each plan creates its test file as part of the same task that creates the source, with automated `<verify>` running tests immediately, and (3) native Swift code (ActivityKit, SwiftUI) cannot be tested with Jest regardless. The Nyquist sampling rate is maintained: tests run after every task commit.
</objective>
