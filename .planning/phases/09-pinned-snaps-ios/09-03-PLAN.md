...
must_haves:
  truths:
    ...
    - "SnapPreviewScreen shows pin toggle between Polaroid and send button only in one-on-one conversations"
...
<task type="auto">
  ...
  <action>
    ...
    2. Modify `src/screens/SnapPreviewScreen.js`:
       ...
       - **One-on-one conversation guard (locked decision):** Determine if this is a 1:1 conversation by checking the conversation participants. Derive `isOneOnOne` from the conversation document's `participants` array:
         ```javascript
         const isOneOnOne = conversation?.participants?.length === 2;
         ```
         If `participants` is not directly available on the screen, derive it from the route params or conversation context. The pin toggle MUST NOT render in multi-recipient sends. When `isOneOnOne` is false, treat `pinToScreen` as false regardless of toggle state.
       - Modify `handleSend`:
         - Pass `{ pinToScreen: pinEnabled && isOneOnOne }` as options to `uploadAndSendSnap`:
           ```javascript
           const result = await uploadAndSendSnap(conversationId, user.uid, photoUri, caption || null, { pinToScreen: pinEnabled && isOneOnOne });
           ```
       - In JSX, between the Polaroid frame `</Animated.View>` and the footer `<View style={screenStyles.footer}`:
         - Add the PinToggle component (only when `pinLoaded` is true AND `isOneOnOne` is true):
           ```jsx
       ...
       - Add `pinEnabled` and `isOneOnOne` to the `handleSend` dependency array
...
9. PinToggle only renders when `isOneOnOne` is true (one-to-one conversations only, per locked decision)
10. `pinToScreen` forced false when `isOneOnOne` is false, regardless of toggle state
11. `npm run lint` passes on all modified files
</verification>
