...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/services/firebase/notificationService.js"
      provides: "handleNotificationTapped extracts pinned snap data for Live Activity deep link"
...
<task type="auto">
  ...
  <action>
    ...
    2. Modify `src/services/firebase/notificationService.js` — `handleNotificationTapped`:
       - No changes needed for Live Activity deep link — the Live Activity's `widgetURL` handles tap-to-open via the app's URL scheme. The existing deep linking config in AppNavigator handles `lapse://messages/{conversationId}`.
       - But verify the snap notification tap handler still works correctly when pinned fields are present in notification data (it should, since it only reads `type` and `senderId`).

    3. Modify `src/components/SnapViewer.js`:
    ...
    4. Extend `__tests__/services/snapService.test.js`:
