...
must_haves:
  ...
  artifacts:
    - path: 'src/components/SnapBubble.js'
      provides: 'Snap message bubble with unopened/opened/sending states'
      min_lines: 80
...
Props: `message`, `isCurrentUser`, `showTimestamp`, `onPress` (opens SnapViewer for unviewed snaps), `isPending` (for optimistic local snaps)

**Three visual states (per locked user decisions):**

1. **Sending** (`isPending === true`):
...
2. **Unopened** (`message.viewedAt === null && !isPending`):
...
3. **Opened** (`message.viewedAt !== null`):
...
- Mock PixelIcon and SnapProgressRing
  ...
  <done>SnapBubble renders three distinct states (sending/unopened/opened) with correct visual treatment. SnapProgressRing provides amber circular animation. All SnapBubble tests pass.</done>
  </task>
...
<verification>
- SnapBubble renders correctly for all three states (sending, unopened, opened)
