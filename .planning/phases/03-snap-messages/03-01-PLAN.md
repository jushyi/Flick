...
- Test error handling: each function returns `{ success: false }` on error
  ...
  <done>snapService.js exports uploadAndSendSnap, markSnapViewed, getSignedSnapUrl. All tests pass. Functions follow project service pattern with { success, error } returns.</done>
  </task>
