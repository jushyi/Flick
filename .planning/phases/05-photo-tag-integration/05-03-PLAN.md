...
<objective>
Add attribution display to FeedPhotoCard and PhotoDetailScreen for reshared photos, update notification navigation to route tagged_photo push notifications to DM conversations, and add "Add to feed" functionality within PhotoDetailScreen when viewing a tagged photo.
...
Output: Modified FeedPhotoCard with attribution, PhotoDetailScreen with attribution + "Add to feed", updated notification handler, tests.
</objective>
...
<task type="auto">
  <name>Task 1: Attribution display in FeedPhotoCard + PhotoDetailScreen</name>
...
This requires knowing whether the current photo context came from a tagged photo message. Two approaches:

- Option A (preferred): Check if the photo has been added via a `taggedPhotoContext` param passed when opening PhotoDetail from TaggedPhotoBubble. This param carries `{ messageId, conversationId, addedToFeedBy }`.
- Option B: Check if photo itself has attribution (but this only exists AFTER adding, not before).

Implement Option A:

1. When TaggedPhotoBubble's onPress opens PhotoDetail, pass `taggedPhotoContext: { messageId, conversationId, photoId, addedToFeedBy: message.addedToFeedBy }` in navigation params.
2. In PhotoDetailScreen, read `taggedPhotoContext` from route params or photo context.
3. If `taggedPhotoContext` exists and current user is NOT the photo owner (tagger):
   ...
   - On press: call `addTaggedPhotoToFeed(photoId, conversationId, messageId)` from photoTagService
   - After success: button becomes "Added to feed" (disabled)
4. Position the button in the action bar area of PhotoDetailScreen (near reactions/comments section).
...
<verify>
<automated>cd "C:/Users/maser/Lapse Clone" && npx jest --testPathPattern="FeedPhotoCard" --bail --no-coverage 2>&1 | tail -15</automated>
...
<task type="auto">
  <name>Task 2: Notification navigation update + FeedPhotoCard attribution tests</name>
  <files>src/services/firebase/notificationService.js, __tests__/components/FeedPhotoCard.test.js</files>
  <action>
...
<verify>
<automated>cd "C:/Users/maser/Lapse Clone" && npx jest **tests**/components/FeedPhotoCard.test.js --bail --no-coverage 2>&1 | tail -15</automated>
<manual>Attribution tests pass. Notification handler correctly routes tagged_photo to Conversation screen.</manual>
</verify>
<done> - handleNotificationTapped routes 'tagged' and 'tagged_photo' types to Conversation screen with conversationId and friendProfile - Old Activity screen routing for tags removed - 5 FeedPhotoCard attribution tests covering rendering, absence, press handler, order, icon - All tests pass
</done>
</task>
...
cd "C:/Users/maser/Lapse Clone" && npx jest --testPathPattern="tagged|photoTag|TaggedPhoto|FeedPhotoCard" --bail --no-coverage
