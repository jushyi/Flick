...
- Only render StreakIndicator when `streakState` is not `'default'` (no icon clutter for conversations without any streak activity). If streakState is `'default'`, do not render the indicator at all.
- This keeps the header clean for non-streak conversations while showing streak status for active ones.
...
- ConversationHeader conditionally renders StreakIndicator for non-default streaks
