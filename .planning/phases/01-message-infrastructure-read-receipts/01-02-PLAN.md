...
must_haves:
  ...
  key_links:
    ...
    - from: 'src/components/ConversationRow.js'
      ...
      via: 'friendProfile prop (from useMessages cache) must include readReceiptsEnabled field'
      pattern: 'friendProfile.*readReceiptsEnabled'
    - from: 'src/screens/ConversationScreen.js'
...
<task type="auto">
  <name>Task 1: Create test scaffolds for ReadReceiptIndicator and ConversationRow</name>
  <files>
    __tests__/components/ReadReceiptIndicator.test.js
    __tests__/components/ConversationRow.test.js
  </files>
...
- `it('shows "Sent" (not "Seen") when either user has readReceiptsEnabled === false')` -- `friendProfile.readReceiptsEnabled = false`, friend has read, assert "Sent".
...
11. NOTE: These tests will initially FAIL (RED state) because the implementation doesn't exist yet. That is expected. Tasks 2 and 3 will make them pass.
    </action>
    <verify>
    <automated>npx jest **tests**/components/ReadReceiptIndicator.test.js **tests**/components/ConversationRow.test.js --no-coverage 2>&1 | tail -10</automated>
...
7. Replace the current `getPreviewText()` logic with a new implementation that uses the Snapchat-style status words per CONTEXT.md:
   ...
   - **When current user sent the last message** (`lastMessage.senderId === currentUserId`):
     ...
     - **Privacy gate:** `const showReadStatus = currentUserProfile?.readReceiptsEnabled !== false && friendProfile?.readReceiptsEnabled !== false`. For the current user's profile, accept it as a new prop (see step 8) OR read from `conversation` metadata. Since ConversationRow does not currently receive `currentUserProfile`, use the `readReceiptsEnabled` value from the existing `currentUserId`-based data. **Simplest approach:** Accept a new `currentUserReadReceipts` boolean prop (default `true`), passed from MessagesScreen where `userProfile` is available via `useAuth`. Alternatively, the privacy check can rely solely on `friendProfile.readReceiptsEnabled` for the friend side, and accept a `readReceiptsEnabled` prop for the current user side.
...
8. If a new `currentUserReadReceipts` prop is added, update MessagesScreen's `renderConversation` to pass it: `currentUserReadReceipts={userProfile?.readReceiptsEnabled !== false}` (where `userProfile` comes from `useAuth()`). This is a one-line change in MessagesScreen.
9. Handle the `lastMessage?.type` field gracefully: default to `'text'` when missing (`const msgType = lastMessage?.type || 'text'`).
10. Conversation list status transitions should be instant (no animation per CONTEXT.md).

**Unread count badge (READ-01 conversation list view):**

11. Find the existing unread indicator in ConversationRow. Per research, there's a simple 8x8 cyan dot (`unreadDot` style).
12. Replace the simple dot with an `UnreadBadge` sub-component:
...
13. Remove or replace the old `unreadDot` style/component.
    </action>
    <verify>
