...
must_haves:
  ...
  artifacts:
    ...
    - path: 'src/screens/ConversationScreen.js'
      provides: 'Passes conversationId to MessageBubble and handles taggedPhotoContext navigation to PhotoDetail'
  ...
  key_links:
    - from: 'src/screens/ConversationScreen.js'
      ...
      via: 'renderMessage passes conversationId prop'
      pattern: 'conversationId.*MessageBubble'
...
<objective>
Build the TaggedPhotoBubble component for rendering tagged photo messages in DM conversations, the photoTagService client-side service for the add-to-feed flow, the MessageBubble delegation, and the ConversationScreen wiring to pass conversationId and handle navigation to PhotoDetail with taggedPhotoContext.

Purpose: Users see tagged photo messages as large, distinct photo cards in conversations with an inline "Add to feed" action button. The full prop chain from ConversationScreen through MessageBubble to TaggedPhotoBubble is wired correctly.

Output: TaggedPhotoBubble component + styles, photoTagService, MessageBubble delegation, ConversationScreen wiring, tests.
</objective>
...
<task type="auto">
  <name>Task 1: Create TaggedPhotoBubble component + styles + photoTagService + MessageBubble delegation + ConversationScreen wiring</name>
  <files>src/components/TaggedPhotoBubble.js, src/styles/TaggedPhotoBubble.styles.js, src/services/firebase/photoTagService.js, src/components/MessageBubble.js, src/screens/ConversationScreen.js</files>
  <action>
...
Props: `message, isCurrentUser, showTimestamp, onPress, reactions, onReactionPress, currentUserId, conversationId`
...
6. "Add to feed" button:
   ...
   - On press: set `isAdding = true`, call `addTaggedPhotoToFeed(message.photoId, conversationId, message.id)`. The `conversationId` is passed as a prop from ConversationScreen through MessageBubble.
...
**Part D: Extend MessageBubble.js and ConversationScreen.js**
...
3. Add `conversationId` to MessageBubble's expected props. It will be passed from ConversationScreen.

**ConversationScreen changes:**

1. In ConversationScreen's `renderMessage` / `renderItem` callback (where MessageBubble is rendered), pass `conversationId` as a prop to MessageBubble:

   ```jsx
   <MessageBubble
     {...existingProps}
     conversationId={conversationId} // conversationId is already available in ConversationScreen from route params or useConversation
   />
   ```

   The `conversationId` value is already available in ConversationScreen -- it comes from route params (`route.params.conversationId`) or is derived from participant IDs. Pass it through to MessageBubble so TaggedPhotoBubble can use it for the `addTaggedPhotoToFeed` call.

2. In the `onPress` handler for messages (or in a new handler passed to TaggedPhotoBubble via MessageBubble), handle tagged photo message taps to navigate to PhotoDetail with `taggedPhotoContext`:

   When a tagged_photo message's onPress fires, navigate to PhotoDetail and include `taggedPhotoContext` in the navigation params:

   ```javascript
   if (message.type === 'tagged_photo') {
     navigation.navigate('PhotoDetail', {
       photo: { id: message.photoId, imageURL: message.photoURL, userId: message.photoOwnerId },
       taggedPhotoContext: {
         messageId: message.id,
         conversationId: conversationId,
         photoId: message.photoId,
         addedToFeedBy: message.addedToFeedBy || {},
       },
     });
     return;
   }
   ...
   This ensures PhotoDetailScreen (Plan 05-03) can show the "Add to feed" button with all required context.
...
<manual>TaggedPhotoBubble component renders without errors. MessageBubble delegation works for tagged_photo type. ConversationScreen passes conversationId and navigates to PhotoDetail with taggedPhotoContext.</manual>
</verify>
<done> - TaggedPhotoBubble renders large photo card with header text, photo, and "Add to feed" button (recipient only) - Distinct visual styling differentiates from snaps and regular messages - Caption NOT shown on card - Sender sees same large card (no compact version) - photoTagService wraps httpsCallable with { success, error } pattern - MessageBubble delegates to TaggedPhotoBubble for type:tagged_photo (after all hooks) - ConversationScreen passes conversationId prop to MessageBubble - ConversationScreen navigates to PhotoDetail with taggedPhotoContext params for tagged_photo messages
</done>
</task>
...
- ConversationScreen passes conversationId to MessageBubble
- ConversationScreen navigates to PhotoDetail with taggedPhotoContext for tagged_photo taps
