...
must_haves:
  truths:
    ...
    - 'markConversationRead only writes readReceipts when there are unread messages (first-read-only semantics) and app is in foreground'
...
6. **Foreground-only guard (CONTEXT.md locked decision):** Import `AppState` from `react-native`. Gate the `markConversationRead` call so it only fires when `AppState.currentState === 'active'`. Additionally, add an `AppState.addEventListener('change', ...)` listener inside the same useEffect: when app state changes to `'active'` (user returns from background) AND there are unread messages, call `markConversationRead`. This ensures read receipts are never written while the app is backgrounded, even if the conversation screen was left open. Clean up the AppState listener in the useEffect cleanup function.
7. Return `conversationDoc` from the hook alongside existing return values (messages, sendMessage, loadMore, etc.). Do NOT break existing consumers -- add it as a new field in the return object.
8. Clean up the conversation document subscription in the useEffect cleanup function (call the unsubscribe function returned by `onSnapshot`).
9. Import `doc` and `onSnapshot` from `@react-native-firebase/firestore` if not already imported. Import `AppState` from `react-native`.

**Cloud Function - onNewMessage (INFRA-01 preparation + preview text):**

10. Open `functions/index.js`. Find the `onNewMessage` function (documented at lines ~2618-2738 per research).
11. Extend the `lastMessage` object that gets written to the conversation document to include the `type` field from the message data. Currently it may only write `text`, `senderId`, `timestamp`. Ensure `type` is included: `type: messageData.type || 'text'`.
12. This prepares for future message types (reactions, replies, snaps, tagged photos) whose `lastMessage` preview text will be generated by the client side (ConversationRow) based on the `type` field.
13. If the function already writes `type` to `lastMessage`, verify and leave as-is.
14. Use the existing logger pattern in the Cloud Function for any new log statements.
    </action>
    <verify>
    <automated>npx jest **tests**/hooks/useConversation.test.js --no-coverage 2>&1 | tail -5</automated>
    <manual>Review useConversation.js to confirm conversation document onSnapshot subscription exists with cleanup, markConversationRead is gated by unreadCount > 0 AND AppState.currentState === 'active', and AppState listener triggers read on foreground return</manual>
    </verify>
    <done> - useConversation hook subscribes to conversation document via onSnapshot - conversationDoc (including readReceipts) is returned from the hook - markConversationRead is only called when unreadCount > 0 (first-read-only semantics) - markConversationRead is only called when AppState.currentState === 'active' (foreground-only guard) - AppState change listener triggers markConversationRead when app returns to foreground with unread messages - Both subscriptions (onSnapshot + AppState) are properly cleaned up on unmount - onNewMessage Cloud Function includes `type` field in lastMessage object
    </done>
    </task>
...
6. Add test cases:
   ...
   - `it('should NOT call markConversationRead when app is backgrounded')` -- Mock `AppState.currentState` as `'background'`, set up conversationDoc with unreadCount > 0, assert `markConversationRead` was NOT called.
   - `it('should call markConversationRead when app returns to foreground with unread messages')` -- Simulate AppState change from `'background'` to `'active'` with unreadCount > 0, assert `markConversationRead` was called.
   - `it('should clean up AppState listener on unmount')` -- Mock `AppState.addEventListener` to return a subscription with `remove`, unmount hook, assert `remove` was called.
...
9. Mock `AppState` from `react-native`: set `AppState.currentState` as a writable property, mock `AppState.addEventListener` to return `{ remove: jest.fn() }` and capture the callback for testing.
   </action>
   <verify>
   ...
   <done> - messageService tests cover: readReceipts write, success/error returns, missing params - useConversation tests cover: conversation doc subscription, readReceipts exposure, first-read-only guard, foreground-only guard (AppState), AppState foreground return trigger, cleanup (both onSnapshot unsubscribe and AppState listener removal) - All tests pass (both new and existing) - Test patterns match project conventions (mock outside jest.mock, require after mocks, clearAllMocks in beforeEach)
   </done>
   </task>
