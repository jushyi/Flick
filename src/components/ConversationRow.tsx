...
type FirestoreTimestamp = { toDate: () => Date; toMillis?: () => number };

const formatMessageTime = (timestamp: string | Date | FirestoreTimestamp | undefined) => {
  if (!timestamp) return '';
  const date = (timestamp as FirestoreTimestamp).toDate ? (timestamp as FirestoreTimestamp).toDate() : new Date(timestamp as string);
  const now = new Date();
...
const UnreadBadge = ({ count, isSnap = false }: UnreadBadgeProps) => {
...
type Props = {
  ...
  onPress: () => void;
  onLongPress?: () => void;
  onSnapCamera?: (conversationId: string, friendId: string | undefined, displayName: string) => void;
};
